...
import com.github.binitabharati.arachne.test.sanity2.JsonRouteEntry.JsonRouteEntry2;
...
public static final Logger logger = LoggerFactory.getLogger("junitLogger");//prints to arachne.log
		...
		for (JsonRouteEntry2 jsonRouteEntry2 : entry1) {
			if (hostIpList.contains(jsonRouteEntry2.getMcIp())) {
				...
				if (copyExpectedRouteEntry.size() > 0) {
						...
						throw new AssertionFailedError("Following route entries were expected, but NOT found! "+copyExpectedRouteEntry);
	...
	private void testRouteAddition() {
		...
		try {
			compareRoutes("test1ExpectedRoute.json", "TEST-ROUTE-ADDITION");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void testRouteUnreachable() {
		...
		try {
			compareRoutes("test2ExpectedRoute.json", "TEST-ROUTE-UNREACHABLE");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
	...
	private void testStaleRouteDeletion() {
		...
		try {
			...
			compareRoutes("test3ExpectedRoute.json", "TEST-ROUTE-DELETION");
		} catch (Exception e) {
			// TODO Auto-generated catch block
			e.printStackTrace();
		}
